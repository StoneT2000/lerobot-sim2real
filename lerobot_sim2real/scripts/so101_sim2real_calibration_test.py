    ...
    def print_joint_angles(self, joint_positions):
        ...
        for joint_name in self.joint_names:
            if joint_name in joint_positions:
                ...
                calib_data = self.calibration_data[joint_name]
                min_val = (
                    calib_data.min_position
                    if calib_data.min_position != float("inf")
                    else 0
                )
                max_val = (
                    calib_data.max_position
                    if calib_data.max_position != float("-inf")
                    else 0
                )
                print(
                    f"  {joint_name:12}: {angle:8.2f}° (range: {min_val:7.2f}° to {max_val:7.2f}°)"
                )
        print("-" * 50)
